...
class StrategyEngine:
    ...
    def generate_signal(self, df: pd.DataFrame, regime: str = "") -> SignalDecision:
        ...
        # Cap model threshold by configured base threshold so live trading
        # does not get stuck on overly strict metadata thresholds.
        model_th = getattr(self.model, "long_threshold", self.cfg.base_long_threshold)
        long_th = min(model_th, self.cfg.base_long_threshold)

        # Slight regime adjustment in very strong trend.
        if adx >= 35:
            long_th -= 0.02
